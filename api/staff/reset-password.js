...
async function assertOwner(accessToken, env) {
  ...
  if (ownerProfile?.role !== "owner") {
    const error = new Error("Only owners can reset staff passwords");
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!targetUserId) {
      return sendJson(res, 404, { error: "Staff account not found" });
    }

    if (targetProfile?.role === "owner") {
      return sendJson(res, 400, {
        error: "Cannot reset password for owner account",
      });
    }
    ...
    if (updateAuthError) {
      return sendJson(res, 400, {
        error: updateAuthError.message || "Could not reset password",
      });
    }
