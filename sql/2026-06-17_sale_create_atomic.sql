...
-- nửa vời.
--
-- ⚠️ LƯU Ý VẬN HÀNH: client (useFinanceActions.finalizeSale) HIỆN CHƯA gọi RPC
-- này (vẫn chạy luồng từng-bước phía client đã được vá phòng thủ). Sau khi apply
-- migration và kiểm thử trên CSDL thật, mới chuyển finalizeSale sang gọi
-- supabase.rpc('sale_create_atomic', ...) để đạt nguyên tử tuyệt đối.
--
...
BEGIN
  ...
  IF v_paid > 0 THEN
    INSERT INTO public.cash_transactions (id, type, amount, branchid, category, date, description, paymentsource, saleid, recipient, userid)
    VALUES (
  ...
  -- 6) Cộng thống kê khách hàng nguyên tử (#8).
