        ...
        {Object.entries(STATUS_CONFIG).map(([status, config]) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`px-1.5 py-0.5 rounded-full text-[9px] ${isActive ? "bg-white/30 dark:bg-black/20" : "bg-slate-200 dark:bg-slate-700"}`}
              >
                {count}
              </span>
            </button>
...
const EmptyPurchaseOrdersState: React.FC<{ statusFilter: string }> = ({
  statusFilter,
}) => {
  return (
    <div className="flex flex-col items-center justify-center py-16 bg-white dark:bg-slate-800 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
      ...
      <p className="text-slate-500 dark:text-slate-400 font-medium">
        {statusFilter === "all"
          ...
          : `Không có đơn ${STATUS_CONFIG[statusFilter]?.label.toLowerCase()}`}
      </p>
    </div>
  );
};
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2 text-sm">
            ...
            <div className="flex items-center gap-2 text-slate-600 dark:text-slate-400">
              ...
              <span>{po.order_date ? formatDate(po.order_date) : "—"}</span>
            </div>
            <div className="flex items-center gap-2 text-slate-600 dark:text-slate-400">
              <span className="w-24 text-slate-500">Dự kiến:</span>
              <span>{po.expected_date ? formatDate(po.expected_date) : "—"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between md:justify-end gap-6 border-t md:border-t-0 border-slate-100 dark:border-slate-700 pt-4 md:pt-0">
          <div className="text-right">
            ...
            <div className="text-xl font-bold text-blue-600 dark:text-blue-400">
              {formatCurrency(
                po.final_amount || (po.total_amount - (po.discount_amount || 0))
              )}
            </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {filteredOrders.length === 0 ? (
        <EmptyPurchaseOrdersState statusFilter={statusFilter} />
      ) : (
