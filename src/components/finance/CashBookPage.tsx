...
const CashBookTable: React.FC<{ isLoading: boolean; transactions: any[] }> = ({
  isLoading,
  transactions,
}) => (
        ...
        <tbody>
          {transactions.map((tx) => {
            ...
            return (
              ...
              >
                <td className="px-4 py-3 text-sm text-slate-700 dark:text-slate-200">{formatDate(new Date(tx.date), true)}</td>
                ...
                <td className="px-4 py-3 text-sm text-slate-700 dark:text-slate-200">{(tx as any).notes || (tx as any).description || "-"}</td>
                <td className="px-4 py-3 text-sm text-slate-700 dark:text-slate-200">{(tx as any).recipient || "-"}</td>
                <td
  ...
  return (
    <div className="p-3 sm:p-5 lg:p-6 min-h-[calc(100vh-64px)]">
      <div className="app-surface max-w-7xl mx-auto rounded-xl overflow-hidden">
        ...
        <CashBookTable isLoading={isLoading} transactions={sortedTx} />
      </div>
